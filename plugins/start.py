        ...
        try:
            ...
            if str(user.status) == "ChatMemberStatus.BANNED":
                await message.reply_text(
                    "You are Banned. Contact -- Team SPY"
                )
                return 1
            ...
            await message.reply_text(
                f"Something Went Wrong. Contact admins... "
                f"with following message {ggn}"
            )
            return 1
...
async def send_or_edit_help_page(_, message, page_number):
    ...
    prev_button = InlineKeyboardButton(
        "◀️ Previous",
        callback_data=f"help_prev_{page_number}"
    )

    next_button = InlineKeyboardButton(
        "Next ▶️",
        callback_data=f"help_next_{page_number}"
    )
...
@app.on_callback_query(
    filters.regex(r"help_(prev|next)_(\d+)")
)
async def on_help_navigation(client, callback_query):
    action, page_number = callback_query.data.split("_")[1], int(
...
async def terms(client, message):
    terms_text = (
        ...
        "If any user engages in such activities, it is solely their responsibility.\n"
        "✨ Upon purchase, we do not guarantee the uptime, downtime, or the validity of the plan. "
        "__Authorization and banning of users are at our discretion; we reserve the right to ban "
        "or authorize users at any time.__\n"
        "✨ Payment to us **__does not guarantee__** authorization for the /batch command. "
        "All decisions regarding authorization are made at our discretion and mood.\n"
    )
    ...
    buttons = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "📋 See Plans",
                    callback_data="see_plan"
                )
            ],
            [
                InlineKeyboardButton(
                    "💬 Contact Now",
                    url="https://t.me/kingofpatal"
                )
            ],
        ]
    )
...
async def plan(client, message):
    plan_text = (
        ...
        "with any downloads or uploads.\n\n"
        "📜 **Terms and Conditions**: For further details and complete terms and conditions, "
        "please send /terms.\n"
    )
...
async def see_plan(client, callback_query):
    plan_text = (
        ...
        "with any downloads or uploads.\n\n"
        "📜 **Terms and Conditions**: For further details and complete terms and conditions, "
        "please send /terms or click See Terms👇\n"
    )
